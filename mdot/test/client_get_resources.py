...
class MdotClientRequestTest(TestCase):
    ...
    def test_get_all_resources(self):
        ...
        self.assertEqual(type(resources[0].title), type("unicode string"))

        # feature_desc: Make sure that the description is unicode
        self.assertEqual(type(resources[0].feature_desc), type("unicode"))

        # image: Make sure that the url is unicode
        self.assertEqual(type(resources[0].image_url), type("unicode"))
    ...
    def test_get_resources_by_featured(self):
        ...
        response = MDOT().getURL(
            "/api/v1/uwresources/?featured=True",
    ...
    def test_get_resources_by_multiple_attrs(self):
        ...
        response = MDOT().getURL(url, {"Accept": "application/json"})
        # assert a 200 status
        self.assertEqual(response.status, 200)
        comparison_data = json.loads(response.data)
